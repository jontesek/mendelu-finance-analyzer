...
class SourceMetricsCalculator2classes(object):
    ...
    def _calc_metrics_from_matrices(self, company_stats):
        ...
        for source in company_stats:
            ...
            for delay, matrix in company_stats[source].items():
                ...
                total_values_count = matrix['pos_up'] + matrix['neg_down'] + matrix['pos_down'] + matrix['neg_up']
                total_correct_count = matrix['pos_up'] + matrix['neg_down']
                if total_values_count == 0:
                    accuracy = None
                else:
                    accuracy = total_correct_count / total_values_count
                d_stats['accuracy'] = accuracy
                #print company_stats[source][delay]
                #print accuracy
                # Precision
