...
from DataAnalyzer.src.classes.LexiconReader import LexiconReader
...
class SentimentIntensityAnalyzer(object):
    ...
    """
    def __init__(self, lexicon_name=False):
        """
        Create a sentiment dictionary (word -> polarity score).
        """
        if lexicon_name:
            # If entered, read other sentiment dictionary.
            lex_reader = LexiconReader()
            self.lexicon = lex_reader.get_dictionary(lexicon_name)
            self.lexicon_name = lexicon_name
        else:
            # Default - use vader lexicon (located in this folder).
            self.lexicon_file = os.path.join(os.path.dirname(__file__), 'vader_sentiment_lexicon.txt')
            self.lexicon = self.make_lex_dict()
            self.lexicon_name = 'vader'
